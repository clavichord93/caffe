...
template <typename Dtype>
__global__ void depthwise_backward_data_gpu_cuda_kernel(const int n,
...
template <typename Dtype>
void depthwise_backward_data_gpu_cuda(const Dtype* data_out, const Dtype* weight,
  ...
  depthwise_backward_data_gpu_cuda_kernel<Dtype>
...
template void depthwise_backward_data_gpu_cuda<float>(const float* data_out,
    const float* weight, float* data_in, const int batch, const int channels,
    const int height, const int width, const int multiplier, const int kernel_h,
    const int kernel_w, const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w);
template void depthwise_backward_data_gpu_cuda<double>(const double* data_out,
...
template <typename Dtype>
__global__ void depthwise_backward_filter_gpu_cuda_kernel(const int n,
    const Dtype* data_out, const Dtype* data_in, Dtype* weight,
    const int channels, const int height, const int width, const int multiplier,
    const int kernel_h, const int kernel_w, const int pad_h, const int pad_w,
    const int stride_h, const int stride_w, const int dilation_h,
    const int dilation_w, const int height_out, const int width_out) {
  CUDA_KERNEL_LOOP(index, n) {
    ...
    data_in_ptr += ((batch * channels + c_in) * height + h_offset) * width +
        w_offset;
    Dtype* weight_ptr = weight;
    weight_ptr += c_out * kernel_h * kernel_w;
    for (int k = 0; k < multiplier; ++k) {
      Dtype val = *data_out_ptr;
      for (int i = 0; i < kernel_h; ++i) {
        for (int j = 0; j < kernel_w; ++j) {
          int h_in = h_offset + i * dilation_h;
          int w_in = w_offset + j * dilation_w;
          if (h_in >= 0 && w_in >= 0 && h_in < height && w_in < width) {
            atomicAdd(weight_ptr + i * kernel_w + j,
                data_in_ptr[i * dilation_h * width + j * dilation_w] * val);
          }
        }
      }
      data_out_ptr += height_out * width_out;
...
template <typename Dtype>
void depthwise_backward_filter_gpu_cuda(const Dtype* data_out, 
    const Dtype* data_in, Dtype* weight, const int batch, const int channels,
    const int height, const int width, const int multiplier, const int kernel_h,
    const int kernel_w, const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w) {
  int height_out = (height + 2 * pad_h - (dilation_h * (kernel_h - 1) + 1)) /
  ...
  int width_out = (width + 2 * pad_w - (dilation_w * (kernel_w - 1) + 1)) /
      stride_w + 1;
  int num_kernels = batch * channels * height_out * width_out;
  depthwise_backward_filter_gpu_cuda_kernel<Dtype>
      <<<CAFFE_GET_BLOCKS(num_kernels), CAFFE_CUDA_NUM_THREADS>>>(
      num_kernels, data_out, data_in, weight, channels, height, width,
      multiplier, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w,
      dilation_h, dilation_w, height_out, width_out);
  CUDA_POST_KERNEL_CHECK;
}

template void depthwise_backward_filter_gpu_cuda<float>(const float* data_out,
    const float* data_in, float* weight, const int batch, const int channels,
    const int height, const int width, const int multiplier, const int kernel_h,
    const int kernel_w, const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w);
template void depthwise_backward_filter_gpu_cuda<double>(const double* data_out,
    const double* data_in, double* weight, const int batch, const int channels,
    const int height, const int width, const int multiplier, const int kernel_h,
    const int kernel_w, const int pad_h, const int pad_w, const int stride_h,
    const int stride_w, const int dilation_h, const int dilation_w);
